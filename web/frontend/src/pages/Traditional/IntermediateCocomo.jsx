...
function IntermediateCocomo() {
    ...
    const save = () => {
        const data = {
            ...
            totalKLOC: totalLOC,
            effortApplied: effortApplied.toFixed(2),
            avgStaffSize:Math.floor(effortApplied/duration),
            cost: (costPerPersonMonth * effortApplied).toFixed(2),
    ...
    const fetchSavedData = () => {
        const q = query(collection(database, "intermediateCocomo"));
    ...
    const openModal = () => {
      ...
      const deleteRow = async (index) => {
        ...
        try {
          await deleteDoc(doc(database, "intermediateCocomo", docId));
    ...
    <tbody>
      {savedData.map((data, index) => (
        <tr key={index}>
          ...
          <td>{data.duration}</td>
          <td>{data.costPerPersonMonth}</td>
          <td>{data.totalKLOC}</td>
          <td>{data.effortApplied}</td>
          <td>{Math.floor(data.effortApplied / data.duration)}</td>
          <td>{Math.floor(data.costPerPersonMonth * data.effortApplied)}</td>
